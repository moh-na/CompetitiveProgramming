...
double mnmx(int src,int dest){
    ...
    while(!pq.empty()){
        ...
        double pc=t.ff*-1;
        int node=t.ss;
        if(dcmp(ans[node],pc)==-1) continue;

        for(auto child: adjlist[node]){


            if(ans[child.second]>max(child.ff.ff,pc)){

                ans[child.second]=max(child.ff.ff,pc);
                pq.push({ans[child.ss]*-1,child.ss});
            }
...
int main(){
    ...
    while(cin>>n>>e>>s>>t){
        ...
        cout<<endl;
        cout<<setprecision(1)<<fixed<<shpath.second+EPS<< " "<<mm+EPS<<endl;
    }
}
