...
void flip(char dial, int direction,char* state){
    char *x=sto[++last];
    ...
    while(!q.empty()){
            ...
            for(int j=0; j<2; j++){
                for(int i=0; i<4; i++){
                   ...
                   flip('A'+i,j,sto[t]);
                    if (visited.wordExist(sto[last])){last--; continue;}
                    else{
                        depth[last]=depth[t]+1;

                        if(depth[last]>9) {last--; continue;}
                        visited.insert(sto[last],depth[last]);
